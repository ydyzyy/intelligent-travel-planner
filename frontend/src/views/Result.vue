      ...
      <div class="main-content">
        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            ...
            <!-- 性能统计 -->
            <a-card id="performance" title="⏱️ 性能统计" :bordered="false" class="performance-card">
              <a-descriptions :column="2" bordered size="small">
                <a-descriptions-item label="景点搜索">
                  <a-tag color="blue">{{ tripPlan.performance?.poi_search || 0 }}秒</a-tag>
                </a-descriptions-item>
                <a-descriptions-item label="天气查询">
                  <a-tag color="green">{{ tripPlan.performance?.weather_query || 0 }}秒</a-tag>
                </a-descriptions-item>
                <a-descriptions-item label="酒店搜索">
                  <a-tag color="orange">{{ tripPlan.performance?.hotel_search || 0 }}秒</a-tag>
                </a-descriptions-item>
                <a-descriptions-item label="LLM生成">
                  <a-tag color="purple">{{ tripPlan.performance?.llm_generation || 0 }}秒</a-tag>
                </a-descriptions-item>
                <a-descriptions-item label="数据解析">
                  <a-tag color="cyan">&lt;1秒</a-tag>
                </a-descriptions-item>
                <a-descriptions-item label="总耗时">
                  <a-tag color="red">{{ tripPlan.performance?.total_time || 0 }}秒</a-tag>
                </a-descriptions-item>
              </a-descriptions>
              <p class="performance-tip">
                💡 LLM生成是主要耗时环节，我们正在持续优化以提升体验
              </p>
            </a-card>
          </div>
              ...
              <a-card size="small" class="weather-card">
                ...
                <div class="weather-info-row">
                  ...
                  <div>
                    ...
                    <div class="weather-value">{{ item.day_weather }} {{ formatTemperature(item.day_temp) }}°C</div>
                  </div>
                </div>
                <div class="weather-info-row">
                  ...
                  <div>
                    ...
                    <div class="weather-value">{{ item.night_weather }} {{ formatTemperature(item.night_temp) }}°C</div>
                  </div>
                </div>
...
// 性能统计(从后端获取的真实数据)
const estimatedLlmTime = ref(0)  // LLM生成时间(秒)
const estimatedTotalTime = ref(0)  // 总耗时(秒)
...
onMounted(async () => {
  ...
  if (route.query.plan) {
    try {
      tripPlan.value = JSON.parse(route.query.plan as string)
      console.log('✅ 从URL参数加载行程计划:', tripPlan.value)
    } catch (error) {
    ...
    // 兼容旧版本:从sessionStorage读取
    const data = sessionStorage.getItem('tripPlan')
    if (data) {
      tripPlan.value = JSON.parse(data)
      console.log('✅ 从sessionStorage加载行程计划')
    }
  }
  
  if (tripPlan.value) {
    // 使用后端返回的真实性能数据
    if (tripPlan.value.performance) {
      console.log('⏱️ 真实性能统计:', tripPlan.value.performance)
    } else {
      console.warn('⚠️ 后端未返回性能统计数据')
    }
    
    console.log('✅ 加载行程计划:', {
      city: tripPlan.value.city,
      days: tripPlan.value.days.length,
      hasPerformance: !!tripPlan.value.performance
    })
...
const formatTemperature = (temp: any): string => {
  // 调试日志
  console.log('🌡️ 温度数据:', temp, '类型:', typeof temp)
  
  if (temp === undefined || temp === null || temp === '') {
    console.warn('⚠️ 温度数据为空')
    return 'N/A'
  }
  ...
  if (typeof temp === 'string') {
    // 移除单位符号
    const cleaned = temp.replace(/[°℃Cc]/g, '').trim()
    ...
    if (!isNaN(parsed)) {
      console.log('✅ 解析温度:', parsed)
      return parsed.toString()
    }
    
    console.warn('⚠️ 无法解析温度字符串:', temp)
    return 'N/A'
  }
  ...
  if (typeof temp === 'number') {
    if (temp === 0) {
      console.warn('⚠️ 温度为0,可能是数据问题')
    }
    return temp.toString()
  }
  
  console.warn('⚠️ 未知的温度数据类型:', temp)
  return 'N/A'
}
...
const initMap = async () => {
  try {
    ...
    if (!amapKey || amapKey === 'your_amap_web_js_key_here') {
      console.warn('⚠️ 高德地图API Key未配置，地图功能将不可用')
      message.warning('地图功能需要配置高德地图API Key，请联系管理员')
      
      // 显示提示信息的占位图
      const container = document.getElementById('amap-container')
      if (container) {
        container.innerHTML = `
          <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; background: #f5f7fa; color: #999;">
            <div style="font-size: 48px; margin-bottom: 16px;">🗺️</div>
            <div style="font-size: 16px; margin-bottom: 8px;">地图功能未配置</div>
            <div style="font-size: 12px;">请在 .env 文件中配置 VITE_AMAP_WEB_JS_KEY</div>
          </div>
        `
      }
      return
    ...
    // 获取第一个景点的位置作为地图中心点
    let centerPoint = [116.397128, 39.916527] // 默认北京
    if (tripPlan.value && tripPlan.value.days.length > 0) {
      const firstAttraction = tripPlan.value.days[0].attractions[0]
      if (firstAttraction && firstAttraction.location) {
        centerPoint = [firstAttraction.location.longitude, firstAttraction.location.latitude]
      }
    }
    ...
    message.error('地图加载失败，请检查API Key是否正确')
    
    // 显示错误信息
    const container = document.getElementById('amap-container')
    if (container) {
      container.innerHTML = `
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; background: #fff2f0; color: #ff4d4f;">
          <div style="font-size: 48px; margin-bottom: 16px;">❌</div>
          <div style="font-size: 16px; margin-bottom: 8px;">地图加载失败</div>
          <div style="font-size: 12px;">${error instanceof Error ? error.message : '未知错误'}</div>
        </div>
      `
    }
  }
...
const addAttractionMarkers = (AMap: any) => {
  ...
  allAttractions.forEach((attraction, index) => {
    const marker = new AMap.Marker({
      ...
      label: {
        content: `<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 6px 10px; border-radius: 6px; font-size: 13px; font-weight: bold; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">${index + 1}</div>`,
    ...
    const infoWindow = new AMap.InfoWindow({
      content: `
        <div style="padding: 12px; max-width: 300px;">
          <h4 style="margin: 0 0 10px 0; color: #667eea;">🏛️ ${attraction.name}</h4>
          <p style="margin: 6px 0; font-size: 13px;"><strong>📍 地址:</strong> ${attraction.address}</p>
          <p style="margin: 6px 0; font-size: 13px;"><strong>⏱️ 游览时长:</strong> ${attraction.visit_duration}分钟</p>
          <p style="margin: 6px 0; font-size: 13px;"><strong>📝 描述:</strong> ${attraction.description}</p>
          <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee;">
            <span style="background: #667eea; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px;">第${attraction.dayIndex + 1}天 景点${attraction.attrIndex + 1}</span>
          </div>
        </div>
      `,
      offset: new AMap.Pixel(0, -35)
  ...
  allHotels.forEach((hotel, index) => {
    const marker = new AMap.Marker({
      ...
      title: hotel.name,
      label: {
        content: `<div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 6px 10px; border-radius: 6px; font-size: 12px; font-weight: bold; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">🏨</div>`,
        offset: new AMap.Pixel(0, -35)
      }
    })
    ...
    const infoWindow = new AMap.InfoWindow({
      content: `
        <div style="padding: 12px; max-width: 300px;">
          <h4 style="margin: 0 0 10px 0; color: #f5576c;">🏨 ${hotel.name}</h4>
          <p style="margin: 6px 0; font-size: 13px;"><strong>📍 地址:</strong> ${hotel.address}</p>
          <p style="margin: 6px 0; font-size: 13px;"><strong>💰 价格:</strong> ${hotel.price_range || '未提供'}</p>
          <p style="margin: 6px 0; font-size: 13px;"><strong>⭐ 评分:</strong> ${hotel.rating || '未评分'}</p>
          <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee;">
            <span style="background: #f5576c; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px;">第${hotel.dayIndex + 1}天 住宿</span>
          </div>
        </div>
      `,
      offset: new AMap.Pixel(0, -35)
...
const drawRoutes = (AMap: any, attractions: any[]) => {
  ...
  // 为每天的景点绘制路线
  Object.entries(dayGroups).forEach(([dayIndex, dayAttractions]: [string, any], colorIndex) => {
    ...
    if (dayAttractions.length > 0) {
      const firstAttr = dayAttractions[0]
      const dayMarker = new AMap.Marker({
        position: [firstAttr.location.longitude, firstAttr.location.latitude],
        content: `<div style="background: ${routeColors[colorIndex % routeColors.length]}; color: white; padding: 8px 12px; border-radius: 20px; font-size: 14px; font-weight: bold; box-shadow: 0 2px 8px rgba(0,0,0,0.2); white-space: nowrap;">Day ${parseInt(dayIndex) + 1}</div>`,
