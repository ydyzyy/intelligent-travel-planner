        ...
        <a-form-item v-if="loading">
          <div class="loading-container">
            ...
            <p class="elapsed-time">
              ⏱️ 已等待: {{ Math.floor(elapsedTime / 60) }}分{{ elapsedTime % 60 }}秒
            </p>
            <p class="loading-tip">
              💡 提示: AI正在为您规划最佳行程，通常需要60-90秒
            </p>
          </div>
        </a-form-item>
      </a-form>
...
const elapsedTime = ref(0)  // 新增：已等待时间（秒）
let timer: any = null  // 新增：计时器
...
const handleSubmit = async () => {
  ...
  const progressInterval = setInterval(() => {
    if (loadingProgress.value < 90) {
      ...
      if (loadingProgress.value <= 20) {
        ...
        loadingStatus.value = '📋 AI正在生成个性化行程...'
      }
    }
  }, 500)
  ...
  try {
    ...
    setTimeout(() => {
      ...
      router.push({
        name: 'Result',
        query: { plan: JSON.stringify(response.data) }
      })
    }, 500)
  } catch (error: any) {
    clearInterval(progressInterval)
