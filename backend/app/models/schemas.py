...
class Meal(BaseModel):
    ...
    meal_type: Optional[str] = Field(default=None, description="餐饮类型（别名）: 早餐/午餐/晚餐")
    ...
    def normalize_type(cls, v, values):
        """兼容meal_type和type字段"""
        if v is None:
            # 如果type为空，尝试使用meal_type
            meal_type = values.data.get('meal_type') if hasattr(values, 'data') else None
            if meal_type:
                # 转换中文到英文
                type_map = {
                    '早餐': 'breakfast',
                ...
                return type_map.get(meal_type, meal_type)
        return v
...
class WeatherInfo(BaseModel):
    ...
    def parse_temperature(cls, v):
        ...
        高德地图API返回的温度格式可能是:
        - "15" 或 "25" - 单一温度值
        - "15/25" - 最低温/最高温格式(取最高温)
        - "15°C" 或 "25℃" - 带单位的温度
        - "" 或 None - 空值
        """
        print(f"🌡️ 温度解析 - 原始值: {v!r}, 类型: {type(v).__name__}")
        ...
        if isinstance(v, str):
            ...
            # 处理"最低温/最高温"格式,取最高温(白天温度通常取较高的值)
            if '/' in v:
                parts = v.split('/')
                print(f"  📊 分割温度: {parts}")
                try:
...
class TripPlan(BaseModel):
    ...
    performance: Optional[Dict[str, float]] = Field(default=None, description="性能统计数据(秒)")
...
class POIInfo(BaseModel):
    ...
    distance: Optional[str] = Field(default="", description="距离(米)")
